...
class BufferPoolManager {
 ...
 protected:
   ...
   * @return the requested page, nullptr if no free page
   */
  ...
  /**
   * Creates a new page on disk and fetches it to buffer pool.
  ...
  /**
   * Set the entrys which proctected by latch_
   * Use condition: the request page is found from free list or replacer
   *                && latch has been acquired
   */
  void SetHelper1(page_id_t page_id, frame_id_t frame_id) {
    // register P in page_table_
  ...
  /** This latch protects page_table_, free_list, replacer, pin count, page dirty, page id  */
  std::mutex latch_;
  /** This latch protects disk_manager_, only BufferPoolManager can use diskmanager, every time it use diskmanager, it
   * hold latch_ first, then hold the disk_latch_, then release latch_ */
