...
int B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(const KeyType &key, const KeyComparator &comparator) const {
  ...
  while (begin < end) {
    ...
    if (great_equal(comparator, KeyAt(middle), key)) {
      end = middle;
    } else {
      begin = middle + 1;
    }
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &comparator) {
  int pos = GetSize();
  while (pos > 0 && great(comparator, KeyAt(pos - 1), key)) {
    array[pos] = std::move(array[pos - 1]);
    pos--;
  }
  array[pos].first = key;
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveHalfTo(BPlusTreeLeafPage *recipient) {
  ...
  for (int i = 0; i + start < GetSize(); i++) {
    recipient->array[i] = std::move(array[start + i]);
  }
  SetSize(start);
}
...
bool B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType *value, const KeyComparator &comparator) const {
  ...
  if (index < GetSize() && equal(comparator, key, KeyAt(index))) {  // key exists
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveAllTo(BPlusTreeLeafPage *recipient) {
  ...
  for (int i = 0; i < GetSize(); i++) {
    recipient->array[rSize + i] = std::move(array[i]);
  }
  SetSize(0);
}
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveFirstToEndOf(BPlusTreeLeafPage *recipient) {
  ...
  recipient->array[recipient->GetSize() - 1] = std::move(array[0]);
  // right page shift
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveLastToFrontOf(BPlusTreeLeafPage *recipient) {
  // shift right page and insert front to it
  recipient->IncreaseSize(1);
  for (int i = recipient->GetSize() - 1; i > 0; i--) {
    recipient->array[i] = std::move(recipient->array[i - 1]);
  }
  recipient->array[0] = std::move(array[GetSize() - 1]);
  // left page shrink
  IncreaseSize(-1);
}
...
template class BPlusTreeLeafPage<GenericKey<8>, void*, GenericComparator<8>>;
template class BPlusTreeLeafPage<GenericKey<24>, RID, GenericComparator<24>>;
}  // namespace bustub
