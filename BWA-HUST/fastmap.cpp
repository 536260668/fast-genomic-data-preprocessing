...
typedef struct {
	...
	bwaidx_t *idx;  // all the data structures about bwt index is here
...
static void *process(void *shared, int step, void *_data)
{
	...
	int i;
	if (step == 0) {    /* Step 0: Read "reads" from input file */
		ktp_data_t *ret = (ktp_data_t*)calloc(1, sizeof(ktp_data_t));
		int size = 0;
		ret->seqs = bseq_read(aux->actual_chunk_size, &ret->n_seqs, aux->ks, aux->ks2, &size);
		if (ret->seqs == 0) {
			free(ret);
			return 0;
		}
		...
		if (bwa_verbose >= 3)
			fprintf(stderr, "[M::%s] read %d sequences (%d bp)...\n", __func__, ret->n_seqs, size);
		return ret;
	} else if (step == 1) { /* Step 1: Main processing-engine */
		const mem_opt_t *opt = aux->opt;
        FMI_search *fmi = aux->fmi;
        LISA_search<index_t> *lisa = aux->lisa;
		...
		if (opt->flag & MEM_F_SMARTPE) {
			...
			if (n_sep[0]) {
				...
				//mem_process_seqs(&tmp_opt, idx->lbwt, idx->bns, idx->pac, aux->n_processed, n_sep[0], sep[0], 0, aux->kt_for);
                mem_process_seqs2(&tmp_opt, fmi, lisa, aux->n_processed, n_sep[0], sep[0], 0, aux->kt_for);
                for (i = 0; i < n_sep[0]; ++i)
					data->seqs[sep[0][i].id].sam = sep[0][i].sam;
			}
			if (n_sep[1]) {
				tmp_opt.flag |= MEM_F_PE;
				//mem_process_seqs(&tmp_opt, idx->lbwt, idx->bns, idx->pac, aux->n_processed + n_sep[0], n_sep[1], sep[1], aux->pes0, aux->kt_for);
                mem_process_seqs2(&tmp_opt, fmi, lisa, aux->n_processed + n_sep[0], n_sep[1], sep[1], aux->pes0, aux->kt_for);
                for (i = 0; i < n_sep[1]; ++i)
					data->seqs[sep[1][i].id].sam = sep[1][i].sam;
			}
			free(sep[0]); free(sep[1]);
		} // else mem_process_seqs(opt, idx->lbwt, idx->bns, idx->pac, aux->n_processed, data->n_seqs, data->seqs, aux->pes0, aux->kt_for);
        else mem_process_seqs2(opt, fmi, lisa, aux->n_processed, data->n_seqs, data->seqs, aux->pes0, aux->kt_for);
		return data;
	} else if (step == 2) { /* Step 2: Write output */
		int n_seqs = data->n_seqs;
		bseq1_t *seqs = data->seqs;
		aux->n_processed += n_seqs;
		for (i = 0; i < n_seqs; ++i) {
...
//		fprintf(stderr, "       -s INT        look for internal seeds inside a seed with less than INT occ [%d]\n", opt->split_width);
    ...
    fprintf(stderr, "start to load index\n");
#ifdef ENABLE_LISA
    LISA_search<index_t> *lisa;
	...
	//if ((aux.idx = bwa_idx_load_from_disk(argv[optind], BWA_IDX_ALL)) == 0) return 1; // FIXME: memory leak
...
//    strcpy(binary_seq_file,  argv[optind]);
//    strcat(binary_seq_file, ".0123");
...
//
//    fprintf(stderr, "index loaded in %.3f CPU sec\n", cputime() - ctime);
...
//			aux.idx->bns->anns[i].is_alt = 0;
	...
	bwa_print_sam_hdr(aux.fmi->idx->bns, hdr_line);
	aux.actual_chunk_size = fixed_chunk_size > 0? fixed_chunk_size : opt->chunk_size * opt->n_threads;
	kt_for_t kt;
	kt.func = 0;
	aux.kt_for = &kt;
	kt_pipeline(no_mt_io? 1 : 2, process, &aux, 3);
	//kt_for(opt->n_threads, NULL, NULL, 0, 1, aux.kt_for);   //destroy thread pool
	free(hdr_line);
	free(opt);
    //_mm_free(ref_string);
	kseq_destroy(aux.ks);
	...
	// malloc_stats_print(NULL, NULL, "a"); // print memory stats
	return 0;
}
