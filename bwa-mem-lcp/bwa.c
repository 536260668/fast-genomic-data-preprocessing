...
bseq1_t *bseq_read(int chunk_size, int *n_, void *ks1_, void *ks2_, int *size_)
{
...
lbwt_t *bwa_idx_load_lbwt(const char *hint, char *prefix)
{
	char *tmp;
	lbwt_t *lbwt;
	if (prefix == 0) {
	...
	lbwt = lbwt_restore_lbwt(tmp);
	strcat(strcpy(tmp, prefix), ".sa");  // partial suffix array (SA)
	bwt_restore_sa_lambert(tmp, lbwt);
	free(tmp);
	return lbwt;
}
...
bwaidx_t *bwa_idx_load_from_disk(const char *hint, int which)
{
	...
	if (which & BWA_IDX_BWT) idx->lbwt = bwa_idx_load_lbwt(hint, prefix);
...
void bwa_idx_destroy(bwaidx_t *idx)
{
	if (idx == 0) return;
	if (idx->mem == 0) {
		if (idx->lbwt) lbwt_destroy(idx->lbwt);
		if (idx->bns) bns_destroy(idx->bns);
		if (idx->pac) free(idx->pac);
	} else {
		free(idx->lbwt); free(idx->bns->anns); free(idx->bns);
