...
static void mem_collect_intv(const mem_opt_t *opt, const lbwt_t *lbwt, int len, const uint8_t *seq, smem_aux_t *a) {
	int i, k, x = 0, old_n;
	int start_width = 1;
	...
	while (x < len) {
		if (seq[x] < 4) {
			x = bwt_smem1(lbwt, len, seq, x, start_width, &a->mem1, a->tmpv);
			for (i = 0; i < a->mem1.n; ++i) {
				bwtintv_t *p = &a->mem1.a[i];
				int slen = p->readEnd - p->readBegin;//(uint32_t)p->info - (p->info>>32); // seed length
	...
	for (k = 0; k < old_n; ++k) {
		...
		uint32_t start = p->readBegin, end = p->readEnd;
		if (end - start < split_len || p->len > opt->split_width) continue;
		bwt_smem1(lbwt, len, seq, (start + end) >> 1, p->len + 1, &a->mem1, a->tmpv);
		for (i = 0; i < a->mem1.n; ++i)
			if (a->mem1.a[i].readEnd - (a->mem1.a[i].readBegin) >= opt->min_seed_len)
	...
	if (opt->max_mem_intv > 0) {
		...
		while (x < len) {
			if (seq[x] < 4) {
				bwtintv_t m;
				x = bwt_seed_strategy1(lbwt, len, seq, x, opt->min_seed_len, opt->max_mem_intv, &m);
				if (m.len > 0) kv_push(bwtintv_t, a->mem, m);
			} else ++x;
		}
	}
	// sort
	...
	for (i = 0; i < chn->n; ++i) {
		...
		for (j = 0; j < p->n; ++j) {
			...
			err_printf("\t%d;%d;%d,%ld(%s:%c%ld)", p->seeds[j].score, p->seeds[j].len, p->seeds[j].qbeg,
			           (long) p->seeds[j].rbeg, bns->anns[p->rid].name, "+-"[is_rev],
...
mem_chain_v
mem_chain(const mem_opt_t *opt, const lbwt_t *lbwt, const bntseq_t *bns, int len, const uint8_t *seq, void *buf) {
	int i, b, e, l_rep;
	...
	for (i = 0; i < aux->mem.n; ++i) {
		...
		for (k = count = 0; k < p->len && count < opt->max_occ; k += step, ++count) {
			...
			s.qbeg = p->readBegin;
			s.score = s.len = slen;
			if (s.rbeg + p->readEnd - p->readBegin > 2 * lbwt->refLen)
				continue; //condition: cycle rotation, out of boundary
...
// equivalence to mem_collect_intv
void mem_collect_smem(FMI_search *fmi,
                      LISA_search<index_t> *lisa,
                      const mem_opt_t *opt,
                      int len,
                      const uint8_t *seq,
                      smem_aux_t *a, std::vector<SMEM>& matchArray)
{
    int64_t pos = 0;
    int split_len = (int)(opt->min_seed_len * opt->split_factor + .499);
    ...
    while(j < len)
    {
        ...
        for(; j<len; j++)
        {
            if((int) seq[j] > 3)  break;
        }

        Info temp_q;
        temp_q.p = (const char*)(seq + prev_j);
        temp_q.offset = prev_j;
        temp_q.l = j - prev_j;
        temp_q.r = j - prev_j;
    ...
    for(Info& q : lisa_qdb)
    {
        lisa->smem_rmi(q, opt->min_seed_len, matchArray);
    }

    // SMEM kernel 2
    num_smem1 = matchArray.size();

    // Input read dataset for kernel 2
    SMEM *tal_smems = &matchArray[0];
    ...
    for (int64_t i=0; i<num_smem1; i++) {
        ...
        SMEM s = *p;
        Info q_temp;
        q_temp.p =  (const char*)seq;
        q_temp.id = s.rid;
        q_temp.prev_l = q_temp.l = q_temp.r = get_rightmost_seed_index(&matchArray[0], i, num_smem1);
        q_temp.intv = {0, lisa->n};
        q_temp.min_intv =  p->s + 1;// - 1;
        q_temp.mid = (end + start)>>1;
    ...
    lisa->fmi_shrink(lisa_qdb_k2.size(), &lisa_qdb_k2[0], &k2_fmi_out[0], opt->min_seed_len);
    ...
    for (int64_t i=0; i<num_smem2; i++) {
        ...
        assert(q_temp.l >= 0 && q_temp.r <= len);
        q_temp.intv = {0, lisa->n};
    ...
    for(int i=0; i<num_smem2; i++)
    {
        lisa->smem_rmi(lisa_qdb_k2[i], opt->min_seed_len, matchArray);
    }
    num_smem2 = matchArray.size();

#else
#endif
// ********************************************   Kernel 3 *************************
    num_smem3 = fmi->bwtSeedStrategyAllPosOneThreadTwoSteps(seq, len, opt->max_mem_intv, opt->min_seed_len, matchArray);

    int tot_smem = matchArray.size();
    if (tot_smem > 0)
        ks_introsort(mem_intv1, tot_smem, &matchArray[0]);
}

// SMEM + SAL
mem_chain_v
mem_chain2(const mem_opt_t *opt, const bntseq_t *bns, FMI_search *fmi, LISA_search<index_t> * lisa, int len, const uint8_t *seq, void *buf) {
    int i, b, e, l_rep;
    int64_t l_pac = bns->l_pac;
    mem_chain_v chain;
    kbtree_t(chn) * tree;
    ...
    std::vector<SMEM> matchArray;
    matchArray.reserve(8);
    //SMEM * matchArray = (SMEM* ) _mm_malloc(sizeof(SMEM) * 64);
    int num_smem = 0;
    mem_collect_smem(fmi, lisa, opt, len, seq, aux, matchArray);

    //fprintf(stderr,"matchArray.size: %d\n", matchArray.size());
    int size = matchArray.size();
    for (i = 0, b = e = l_rep = 0; i < size; ++i) { // compute frac_rep
        SMEM& p = matchArray[i];
        int sb = p.m, se = p.n + 1;
    ...
    lbwt_t* lbwt = lisa->lbwt;
    for (i = 0; i < size; ++i) {
        SMEM& p = matchArray[i];
        int step, count, slen = p.n + 1 - p.m; // seed length
        int64_t k;
        // if (slen < opt->min_seed_len) continue; // ignore if too short or too repetitive
        ...
        for (k = count = 0; k < p.s && count < opt->max_occ; k += step, ++count) {
            ...
            mem_seed_t s;
            int rid, to_add = 0;
            s.rbeg = tmp.pos = lbwt->sa_low32[p.k + k] + ((3ull & _get_sahigh2(lbwt->sa_high2, p.k + k))
                    << 32);// this is the base coordinate in the forward-reverse reference      // bwt_sa() method is implemented here
            s.qbeg = p.m;
            s.score = s.len = slen;
            if (s.rbeg + p.n - p.m > 2 * lbwt->refLen)
                continue; //condition: cycle rotation, out of boundary
            rid = bns_intv2rid(bns, s.rbeg, s.rbeg + s.len);
            if (rid < 0)
                continue; // bridging multiple reference sequences or the forward-reverse boundary; TODO: split the seed; don't discard it!!!
...
int mem_chain_flt(const mem_opt_t *opt, int n_chn, mem_chain_t *a) {
	...
	for (i = k = 0; i < n_chn; ++i) {
		...
		else a[k++] = *c; // this line free the chain with small weight
	}
	n_chn = k;
	ks_introsort(mem_flt, n_chn, a);//sort chains use chain.weight
	// pairwise chain comparisons
	a[0].kept = 3;
	kv_push(int, chains, 0);// push the 0-th chain
	for (i = 1; i < n_chn; ++i) {
		int large_ovlp = 0;
		for (k = 0; k < chains.n; ++k) {// k must small than or equal to i
			int j = chains.a[k];
			int b_max = chn_beg(a[j]) > chn_beg(a[i]) ? chn_beg(a[j]) : chn_beg(a[i]);
			int e_min = chn_end(a[j]) < chn_end(a[i]) ? chn_end(a[j]) : chn_end(a[i]);
			if (e_min > b_max && (!a[j].is_alt ||
			                      a[i].is_alt)) { // have overlap; don't consider ovlp where the kept chain is ALT while the current chain is primary
		...
		if (k == chains.n) {
			kv_push(int, chains, i);// push the i-th chain
...
int mem_patch_reg(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, uint8_t *query, const mem_alnreg_t *a,
                  const mem_alnreg_t *b, int *_w) {
	int w, score, q_s, r_s;
	double r;
	...
	if (bwa_verbose >= 4)
		printf("* potential hit merge between [%d,%d)<=>[%ld,%ld) and [%d,%d)<=>[%ld,%ld), @ %s; w=%d, r=%.4g\n",
		       a->qb, a->qe, (long) a->rb, (long) a->re, b->qb, b->qe, (long) b->rb, (long) b->re,
		       bns->anns[a->rid].name, w, r);
	...
	q_s = (int) ((double) (b->qe - a->qb) / ((b->qe - b->qb) + (a->qe - a->qb)) * (b->score + a->score) +
	             .499); // predicted score from query
	r_s = (int) ((double) (b->re - a->rb) / ((b->re - b->rb) + (a->re - a->rb)) * (b->score + a->score) +
...
int mem_sort_dedup_patch(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, uint8_t *query, int n,
                         mem_alnreg_t *a) {
	int m, i, j;
	if (n <= 1) return n;
...
static void mem_mark_primary_se_core(const mem_opt_t *opt, int n, mem_alnreg_t *a,
...
int mem_seed_sw(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, int l_query, const uint8_t *query,
                const mem_seed_t *s) {
	int qb, qe, rid;
	...
	x = ksw_align2(qe - qb, (uint8_t *) query + qb, re - rb, rseq, 5, opt->mat, opt->o_del, opt->e_del, opt->o_ins,
	               opt->e_ins, KSW_XSTART, 0);
	free(rseq);
	return x.score;
}

void mem_flt_chained_seeds(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, int l_query, const uint8_t *query,
...
void mem_chain2aln(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, int l_query, const uint8_t *query,
	...
	for (k = c->n - 1; k >= 0; --k) {
		mem_alnreg_t *a;
		s = &c->seeds[(uint32_t) srt[k]];// to select highest score seeds among the seed that not be choosed
		...
		if (s->qbeg) { // left extension
			...
			if (bwa_verbose >= 4) {
				int j;
				printf("*** Left ref:   ");
				for (j = 0; j < tmp; ++j) putchar("ACGTN"[(int) rs[j]]);
				putchar('\n');
				printf("*** Left query: ");
				for (j = 0; j < s->qbeg; ++j) putchar("ACGTN"[(int) qs[j]]);
				putchar('\n');
			}
			a->score = ksw_extend(s->qbeg, qs, tmp, rs, 5, opt->mat, opt->o_del, opt->e_ins,
			                       aw[0], opt->pen_clip5, opt->zdrop, s->len * opt->a, &qle, &tle, &gtle, &gscore,
			                       &max_off[0]);
			if (bwa_verbose >= 4) {
				printf("*** Left extension: prev_score=%d; score=%d; qle=%d; tle=%d; gtle=%d; gscore=%d; bandwidth=%d; max_off_diagonal_dist=%d\n",
				       s->len * opt->a, a->score, qle, tle, gtle, gscore, aw[0], max_off[0]);
				fflush(stdout);
			}
		...
		if (s->qbeg + s->len != l_query) { // right extension
			...
			if (bwa_verbose >= 4) {
				int j;
				printf("*** Right ref:   ");
				for (j = 0; j < rmax[1] - rmax[0] - re; ++j) putchar("ACGTN"[(int) rseq[re + j]]);
				putchar('\n');
				printf("*** Right query: ");
				for (j = 0; j < l_query - qe; ++j) putchar("ACGTN"[(int) query[qe + j]]);
				putchar('\n');
			}
			a->score = ksw_extend(l_query - qe, query + qe, rmax[1] - rmax[0] - re, rseq + re, 5, opt->mat, opt->o_del,
								  opt->e_ins, aw[1], opt->pen_clip3, opt->zdrop, sc0, &qle,
			                       &tle, &gtle, &gscore, &max_off[1]);
			if (bwa_verbose >= 4) {
				printf("*** Right extension: prev_score=%d; score=%d; qle=%d; tle=%d; gtle=%d; gscore=%d; bandwidth=%d; max_off_diagonal_dist=%d\n",
				       sc0, a->score, qle, tle, gtle, gscore, aw[1], max_off[1]);
				fflush(stdout);
			}
...
void mem_aln2sam(const mem_opt_t *opt, const bntseq_t *bns, kstring_t *str, bseq1_t *s, int n, const mem_aln_t *list,
                 int which, const mem_aln_t *m_) {
	int i, l_name;
	...
	if (p->flag & 0x100) { // for secondary alignments, don't write SEQ and QUAL
		...
		if (p->n_cigar && which && !(opt->flag & MEM_F_SOFTCLIP) &&
		    !p->is_alt) { // have cigar && not the primary alignment && not softclip all
...
void
mem_reg2sam(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, bseq1_t *s, mem_alnreg_v *a, int extra_flag,
            const mem_aln_t *m) {
	extern char **mem_gen_alt(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_alnreg_v *a, int l_query,
	            const char *query);
	kstring_t str;
	kvec_t(mem_aln_t) aa;
	int k, l;
...
mem_alnreg_v mem_align1_core(const mem_opt_t *opt, const lbwt_t *lbwt, const bntseq_t *bns, const uint8_t *pac,
	...
	chn = mem_chain(opt, lbwt, bns, l_seq, (uint8_t *) seq, buf);
	...
	for (i = 0; i < regs.n; ++i) {//copy information
		mem_alnreg_t *p = &regs.a[i];
...
// this method is to process only one read
mem_alnreg_v mem_align2_core(const mem_opt_t *opt, FMI_search *fmi, LISA_search<index_t> * lisa,
                             int l_seq, char *seq, void *buf) {
    int i;
    mem_chain_v chn;
    mem_alnreg_v regs;
    const bntseq_t *bns = fmi->idx->bns;
    uint8_t* pac = fmi->idx->pac;

    for (i = 0; i < l_seq; ++i) // convert to 2-bit encoding if we have not done so
        seq[i] = seq[i] < 4 ? seq[i] : nst_nt4_table[(int) seq[i]];

    chn = mem_chain2(opt, bns, fmi, lisa, l_seq, (uint8_t *) seq, buf);
    chn.n = mem_chain_flt(opt, chn.n, chn.a);
    mem_flt_chained_seeds(opt, bns, pac, l_seq, (uint8_t *) seq, chn.n, chn.a);
    if (bwa_verbose >= 4) mem_print_chain(bns, &chn);
...
mem_aln_t mem_reg2aln(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, int l_query, const char *query_,
...
//		w->regs[i] = mem_align1_core(w->opt, w->lbwt, w->bns, w->pac, w->seqs[i].l_seq, w->seqs[i].seq, w->aux[tid]);
//	} else {
//		if (bwa_verbose >= 4) printf("=====> Processing read '%s'/1 <=====\n", w->seqs[i << 1 | 0].name);
//		w->regs[i << 1 | 0] = mem_align1_core(w->opt, w->lbwt, w->bns, w->pac, w->seqs[i << 1 | 0].l_seq,
//		                                      w->seqs[i << 1 | 0].seq, w->aux[tid]);
//		if (bwa_verbose >= 4) printf("=====> Processing read '%s'/2 <=====\n", w->seqs[i << 1 | 1].name);
//		w->regs[i << 1 | 1] = mem_align1_core(w->opt, w->lbwt, w->bns, w->pac, w->seqs[i << 1 | 1].l_seq,
...
static void worker1(void *data, long i, int tid) {
    ...
    if (!(w->opt->flag & MEM_F_PE)) {
        if (bwa_verbose >= 4) printf("=====> Processing read '%s' <=====\n", w->seqs[i].name);
        w->regs[i] = mem_align2_core(w->opt, w->fmi, w->lisa, w->seqs[i].l_seq, w->seqs[i].seq, w->aux[tid]);
    } else {
        if (bwa_verbose >= 4) printf("=====> Processing read '%s'/1 <=====\n", w->seqs[i << 1 | 0].name);
        w->regs[i << 1 | 0] = mem_align2_core(w->opt, w->fmi, w->lisa, w->seqs[i << 1 | 0].l_seq,
                                              w->seqs[i << 1 | 0].seq, w->aux[tid]);
        if (bwa_verbose >= 4) printf("=====> Processing read '%s'/2 <=====\n", w->seqs[i << 1 | 1].name);
        w->regs[i << 1 | 1] = mem_align2_core(w->opt, w->fmi, w->lisa, w->seqs[i << 1 | 1].l_seq,
                                              w->seqs[i << 1 | 1].seq, w->aux[tid]);
    }
}
...
static void worker2(void *data, long i, int tid) {
	extern int
	mem_sam_pe(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_pestat_t pes[4], uint64_t id,
	           bseq1_t s[2], mem_alnreg_v a[2]);
	extern void
	mem_reg2ovlp(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, bseq1_t *s, mem_alnreg_v *a);
	worker_t *w = (worker_t *) data;
	if (!(w->opt->flag & MEM_F_PE)) {
...
void
mem_process_seqs(const mem_opt_t *opt, const lbwt_t *lbwt, const bntseq_t *bns, const uint8_t *pac, int64_t n_processed,
                 int n, bseq1_t *seqs, const mem_pestat_t *pes0, kt_for_t *t) {
	worker_t w;
	mem_pestat_t pes[4];
	double ctime, rtime;
	int i;
	...
	kt_for(opt->n_threads, worker1, &w, (opt->flag & MEM_F_PE) ? n >> 1 : n, 0, t); // find mapping positions
	for (i = 0; i < opt->n_threads; ++i)
	...
	//kt_for(opt->n_threads, worker2, &w, (opt->flag & MEM_F_PE) ? n >> 1 : n, 0, t); // generate alignment
	free(w.regs);
	if (bwa_verbose >= 3)
...
void
mem_process_seqs2(const mem_opt_t *opt, FMI_search* fmi, LISA_search<index_t>* lisa,int64_t n_processed,
                 int n, bseq1_t *seqs, const mem_pestat_t *pes0, kt_for_t *t) {
    worker_t w;
    mem_pestat_t pes[4];
    double ctime, rtime;
